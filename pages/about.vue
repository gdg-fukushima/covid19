<template>
  <div class="About">
    ...
    <StaticCard>
      ...
      {{
        $t(
          '福島県による公式情報と客観的な数値を分かりやすく伝えることで、福島県にお住まいの方や、福島県内に拠点を持つ企業の方、福島県を訪れる方が、現状を把握して適切な対策を取れるようにすることを目的としています。'
        )
      }}
      <br />
      <br />
      {{
        $t(
          '当サイトのサーバー運用費は、Google社よりサポートを受けています。'
        )
      }}
    </StaticCard>
    <StaticCard>
      <h2>{{ $t('Code for Fukushimaとは') }}</h2>
      <p>{{ $t('新型コロナウイルス感染症（COVID-19）の感染拡大に伴い、福島県を中心に県内外で活動しているコミュニティ・個人が集結し結成されました。') }}</p>
      <p>{{ $t('2020/3/18より、有志による当サイトの運用が始まりました。') }}</p>
    </StaticCard>
    <StaticCard>
      <h2>{{ $t('ブラウザ環境について') }}</h2>
      <p>
    ...
    <StaticCard>
      ...
      <p>
        {{
          $t(
            '当サイトに掲載されている情報の正確性については万全を期していますが、製作者は利用者が当サイトの情報を用いて行う一切の行為について責任を負うものではありません。'
          )
        }}
      </p>
      <p>
    ...
    <StaticCard>
      ...
      <i18n
        tag="p"
        path="本サイトで公表しているデータは、{dataSite}より誰でも閲覧が可能になっているものです。"
      >
        <a
          href="https://www.pref.fukushima.lg.jp/sec/21045c/covid19-opendata.html"
          target="_blank"
          rel="noopener"
          place="dataSite"
        >
